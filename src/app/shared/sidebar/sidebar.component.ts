...
export class SidebarComponent implements AfterViewInit {
    ...
    @Input() menuItems: Items[] = [];

    selectedMenuItem: Items | null = null;
    ...
    scripts() {
        ...
        function defaultMenuCollapse() {
            if ($('body').hasClass('menu-collapse') && $(window).width() > 993) {
                ...
                toogleMenuCollapse();
                navigationCollapse();
            } else if ($(window).width() > 993 && $('footer')) {
                $('footer').css('padding-left', '267px');
            }
        }
        ...
        $(() => {
            ...
            var indeterminateCheckbox = <HTMLInputElement>document.getElementById('indeterminate-checkbox');
            ...
            $('.dropdown-submenu').on('mouseenter', () => {
                var $this = $(this);
            ...
            $('.dropdown-submenu').on('mouseleave', () => {
                var $this = $(this);
            ...
            $('#slide-out > li').click(function () {
                if (!$('#slide-out > li.bold.active > div.collapsible-body').is(':visible')) {
                    $('#slide-out > li.bold').removeClass('open');
                    $('#slide-out > li > div.collapsible-body').slideUp({
                        duration: 300
                    });
                }
            });
            ...
            function abrirMenu(children: any) {
                if (children.hasClass('IsMain')) {
                    abrirMenu(children.parent('ul.collapsible-sub'));
                } else if (children.parent('div.collapsible-body').parent('li.bold').length > 0) {
                    children.parent('div.collapsible-body').parent('li.bold').addClass('active');
                    abrirMenu(children.parent('div.collapsible-body').parent('li.bold'));
                } else if (children.parent('ul.collapsible-sub').length > 0) {
                    abrirMenu(children.parent('ul.collapsible-sub'));
                }
            }

            setTimeout(function () {
                abrirMenu($('.sidenav-main li a.active').parent('li.active'));
            });
            ...
            $('.nav-collapsible .navbar-toggler').click(() => {
                ...
                if ($('.nav-collapsible .navbar-toggler i').text() == 'radio_button_unchecked') {
                ...
                if (!$('.sidenav-main.nav-collapsible').hasClass('nav-lock')) {
                    ...
                    setTimeout(function () {
                        ...
                        if ($('.collapsible .open').children().length > 1) {
                            if (!$('.collapsible .open').hasClass('IsMain')) {
            ...
            //Chat search filter
            $('#messages .header-search-input').on('keyup', () => {
    ...
    close() {
        document.addEventListener("DOMContentLoaded", () => {
            if (window.innerWidth < 993) {
                ...
                sidenav[0].classList.add('.nav-collapsed');
            }
        });
    }
